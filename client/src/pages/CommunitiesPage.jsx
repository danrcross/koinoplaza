...
export default function CommunitiesPage() {
  ...
  } = useQuery(GET_USER_COMMUNITIES, {
    variables: { userID },
  });
  const {
    data: otherCommunitiesData,
    loading: otherCommunitiesLoading,
    refetch: refetchOtherCommunities,
  } = useQuery(GET_OTHER_COMMUNITIES);
  // QUERY FOR TESTING
  const {
    data: testCommunitiesData,
    loading: testCommunitiesLoading,
    refetch: refetchTestCommunities,
  ...
  if (userLoading || userCommunitiesLoading || otherCommunitiesLoading)
    return <p>Loading...</p>;
  ...
  const testCommunities = testCommunitiesData?.communities || [];

  const createdCommunitiesCount = userCommunities.filter(
    (community) => community.membership === "Creator"
  ).length;
  const joinedCommunitiesCount =
    userCommunities.length - createdCommunitiesCount;
  ...
  return (
    <>
      ...
      {openLists.mycommunities && (
        <>
          <MyCommunities
            myCommunityData={testCommunities}
